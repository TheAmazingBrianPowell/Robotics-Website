...
<?php
  ...
  foreach (new RecursiveIteratorIterator($di) as $filename => $file) {
    if((substr($filename, -9) == 'index.php' || substr($filename, -4) == 'html' || substr($filename, -3) == 'pdf') && substr($filename, 1, 7) != '/error/' && strpos($filename, '/vendor/') === FALSE && strpos($filename, '/.heroku/') === FALSE && strpos($filename, 'google56830dc2c8d49f6f') === FALSE) {
